...
export default {
  ...
  getters: {
    ...
    commentCount: (state) => (propertyId) =>
      (state.comments[propertyId] || []).length,
  ...
  mutations: {
    ...
    UPDATE_COMMENT(state, { commentId, content, rating }) {
      // 遍歷所有房源的評論
      for (const propertyId in state.comments) {
        const commentIndex = state.comments[propertyId].findIndex(
          (comment) => comment.id === commentId
        );

        if (commentIndex !== -1) {
          // 使用新的數據創建一個新對象，確保響應式更新
          const updatedComment = {
            ...state.comments[propertyId][commentIndex],
            content,
            rating,
          };

          // 替換原來的評論對象
          state.comments[propertyId].splice(commentIndex, 1, updatedComment);
          break;
        }
      }
    },
    ...
    DELETE_REPLY(state, { replyId, commentId, propertyId }) {
      ...
      if (propertyId && commentId && state.comments[propertyId]) {
        ...
        if (
          commentIndex !== -1 &&
          state.comments[propertyId][commentIndex].replyCount > 0
        ) {
          state.comments[propertyId][commentIndex].replyCount -= 1;
        }
      }
    ...
    UPDATE_REPLY(state, { replyId, content }) {
      const replyIndex = state.replies.findIndex(
        (reply) => reply.id === replyId
      );

      if (replyIndex !== -1) {
        state.replies[replyIndex].content = content;
      }
    },
  },
  ...
  actions: {
    ...
    async fetchComments({ commit }, { propertyId, page = 1, perPage = 20 }) {
      ...
      try {
        const response = await apiService.comments.getPropertyComments(
          propertyId,
          { page, perPage }
        );
    ...
    async addComment({ commit }, { propertyId, content, rating }) {
      ...
      try {
        const response = await apiService.comments.createComment(propertyId, {
          content,
          rating,
        });
    ...
    // 更新評論
    async updateComment({ commit }, { commentId, content, rating }) {
      try {
        // 這裡調用 API 來更新評論
        const response = await apiService.comments.updateComment(commentId, {
          content,
          rating,
        });
        ...
        if (response && response.success) {
          // 更新本地狀態
          commit("UPDATE_COMMENT", { commentId, content, rating });
          return true;
        }
    ...
    async fetchReplies({ commit }, { commentId, page = 1, perPage = 50 }) {
      ...
      try {
        const response = await apiService.comments.getReplies(commentId, {
          page,
          perPage,
        });
    ...
    async addReply({ commit, state }, { commentId, content }) {
      try {
        const response = await apiService.comments.createReply(commentId, {
          content,
        });
        ...
        if (propertyId) {
          ...
          if (commentIndex !== -1) {
            const updatedReplyCount =
              (commentsList[commentIndex].replyCount || 0) + 1;
    ...
    async updateReply({ commit }, { replyId, content }) {
      ...
      try {
        const response = await apiService.comments.updateReply(replyId, {
          content,
        });
    ...
    async updateReply({ commit }, { replyId, content }) {
      try {
        ...
        if (response && response.success) {
          // 更新本地狀態
          commit("UPDATE_REPLY", { replyId, content });
          return true;
        }
    ...
    async reportContent(
      { commit },
      { contentType, contentId, reasons, description }
    ) {
      try {
        // 這裡調用 API 來提交舉報
        const response = await apiService.comments.reportContent({
          contentType,
          contentId,
          reasons,
          description,
