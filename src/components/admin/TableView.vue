        ...
        <tbody>
          <tr v-for="row in tableData" :key="getPrimaryKey(row)">
            ...
            <td class="actions-cell">
              ...
              <button @click="deleteRow(row)" class="delete-button">
                刪除
              </button>
            </td>
          </tr>
          <tr v-if="tableData.length === 0">
            <td :colspan="columns.length + 1" class="no-data">無資料</td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          >
            ...
            />
            <small
              >{{ column.type }} {{ column.nullable ? "" : "(必填)" }}</small
            >
          </div>
        </div>
...
<script>
  ...
  export default {
    setup() {
      ...
      const isLocalFilterMode = ref(true); // 設為 true 啟用前端過濾

      const searchableColumns = computed(() => {
        // 排除不適合搜尋的欄位類型，如 BLOB、圖片等
      ...
      const loadTableStructure = async () => {
        try {
          const response = await apiService.admin.getTableStructure(
            tableName.value
          );
      ...
      const loadTableData = async () => {
        try {
          ...
          // 如果不是在搜尋狀態，不添加搜尋參數
          if (searchTerm.value && !isLocalFilterMode.value) {
            params.search = searchTerm.value;
            params.search_field = searchField.value;
            isSearchActive.value = true;
          } else {
            isSearchActive.value = searchTerm.value.trim() !== '';
          }

          const response = await apiService.admin.getTableData(
            tableName.value,
            params
          );
          ...
          // 如果是本地過濾模式，應用過濾器
          if (isLocalFilterMode.value && searchTerm.value.trim() !== '') {
            applyLocalFilter();
          } else {
      ...
      const saveRow = async () => {
        try {
          ...
          await apiService.admin.updateTableRow(
            tableName.value,
            rowId,
            currentRow.value
          );
